...
I thought it'd be easy but having several tweets made it hard somehow: even when playing with chunk options and `purrr::map`-ping and friends, I wasn't able to use `blogdown::shortcode` so I used its insides instead. Still, using Hugo shortcodes is easier than hacking html.

Enjoy reading, and if you can, attend one of the next satRday conferences, for instance in [Amsterdam on September the 1st](http://amsterdam2018.satrdays.org/)! 


```{r}
library("magrittr")
# Good than one can choose glue limits!
glue::glue("{{< tweet [my_satrday_tweets$status_id] >}}",
             .open = "[", .close = "]") %>%
    htmltools::HTML()
