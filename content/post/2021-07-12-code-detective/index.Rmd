...
Sometimes you might also make do with print-debugging i.e. for instance writing `print("coucou !")` to check a part of the code was run, or `print(class(x))` to check an assumption about thing. 
...
The basic idea is that you just replace the `print()` command you were about to right with `browser()`, run the code and voilà! You entered the debugger and can run code line by line, explore options and environment variables, etc.
...
Those can be useful too to read, especially when they start breaking after your experiments I suppose.
...
The simple fact of phrasing your issue might help you solve it.
...
In a package I co-maintain I however remember receiving nice PRs from someone who had just read the [Clean Code book by Robert C. Martin](https://www.goodreads.com/book/show/3735293-clean-code).
...
As much as solving a problem on one's own is nice, asking for help might be the solution!
It is also a skill or a bunch of skills: both how and where to ask for help but also deciding when to ask for help, when it's no longer worth anyone's money to have your keeping working alone on a problem.
...
See also my post on R-hub blog, [_How to get help with R package development? R-package-devel and beyond_](https://blog.r-hub.io/2019/04/11/r-package-devel/), for general venues where to ask help about package development.
