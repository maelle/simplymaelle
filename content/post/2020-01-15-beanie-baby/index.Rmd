...
This has two consequences: I need to calm down a bit, and I am inspired to, well, blog with R Markdown!
As I've just read a [fascinating book about the beanie baby fad](https://www.goodreads.com/book/show/20821185-the-great-beanie-baby-bubble) and as I've seen rvest is getting an update, I've decided to harvest [Beaniepedia](https://beaniepedia.com).
...
```{r}
# I found the right argument via rcorpora::categories() 
# and rcorpora::corpora(category = "animals")
...
head(urls_df$parsed_path)
```

Will "jemima-puddle-duck-the-duck" be a problem?
Also, clearly, we won't detect minions and snowmen as they are not common animals.
Hopefully not.
Now for each parsed path let's see whether it corresponds to any common animal.

```{r}
urls_df %>%
  dplyr::mutate(animals = toString(animals[animals %in% parsed_path[[1]]]))
